...
Function Get-AccessTokenCode($clientId,$secret,$resourceId,$tenantId)
{
  ...
  $Authentication = $Response.Content | ConvertFrom-Json
	...
	# 保存访问令牌
    Set-Content ".\AccessTokenCode.txt" $Authentication.access_token
}
...
#获取访问令牌
If ((Test-Path -Path ".\AccessTokenCode.txt") -ne $false) {
	$accessToken = Get-Content ".\AccessTokenCode.txt"
}
else {
    $accessToken = $null
}
...
If (($accessToken -eq $null) -or ((get-date) - (get-item ".\AccessTokenCode.txt").LastWriteTime).TotalHours -gt 1) 
{
